...
class AnalyticsService:
    ...
    async def submit_spaced_review(
        self, user_id: int, submit: SpacedReviewSubmit
    ) -> Dict[str, Any]:
    ...
    async def get_teacher_class_analytics(self) -> Dict[str, Any]:
        ...
        total_quizzes_res = await self.session.execute(select(func.count(QuizAttempt.id)))
        ...
        hard_questions_res = await self.session.execute(
            select(
                ...
                func.sum(case((QuizAnswer.is_correct == True, 1), else_=0)).label(
                    "correct_answers"
                ),
            )
            ...
            .order_by(
                (
                    func.sum(case((QuizAnswer.is_correct == True, 1), else_=0))
                    * 1.0
