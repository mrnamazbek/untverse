...
class LearningService:
    ...
    def _translation(
        self, translations: Sequence[T], locale: str, entity_name: str, entity_id: int
        ...
        requested = next(
            (
                ...
                if getattr(translation, "locale", None) == locale
            ),
            None,
        )
        if requested:
        ...
        fallback = next(
            (
                translation
                for translation in translations
                if getattr(translation, "locale", None) == "ru"
            ),
            None,
        )
        logger.warning(
    ...
    def _lesson_response(
        self, lesson, completed_lesson_ids: set[int], locale: str
    ) -> LessonResponse:
        translation, fallback_locale = self._translation(
            lesson.translations, locale, "lesson", lesson.id
        )
    ...
    def _topic_response(self, topic, completed_lesson_ids: set[int], locale: str) -> TopicResponse:
        translation, fallback_locale = self._translation(
            topic.translations, locale, "topic", topic.id
        )
        return TopicResponse(
            ...
            lessons=[
                self._lesson_response(lesson, completed_lesson_ids, locale)
    ...
    async def get_courses_hierarchy(
        self, user_id: Optional[int] = None, locale: str = "kk"
    ) -> List[CourseResponse]:
        ...
        if user_id:
            completed_lesson_ids = set(
                await self.course_repo.get_user_completed_lesson_ids(user_id)
            )
    ...
    async def get_topic_detail(
        self, slug: str, user_id: Optional[int] = None, locale: str = "kk"
    ) -> TopicResponse:
        locale = locale if locale in SUPPORTED_LOCALES else "kk"
    ...
    async def get_lesson_detail(
        self, lesson_id: int, user_id: Optional[int] = None, locale: str = "kk"
    ) -> LessonResponse:
        locale = locale if locale in SUPPORTED_LOCALES else "kk"
    ...
    async def complete_lesson(self, user_id: int, lesson_id: int) -> LessonCompleteResponse:
        ...
        if not is_already_completed:
            (
                ...
                new_level,
                leveled_up,
            ) = await self.gamification_service.handle_lesson_completed(
                user_id=user_id, lesson_id=lesson_id, xp_reward=lesson.xp_reward
            )
