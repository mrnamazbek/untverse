...
class SpecificationService:
    ...
    async def get_current_unt_rules(
        ...
        return {
            ...
            "last_verified_at": rule.last_verified_at.isoformat()
            if rule.last_verified_at
            else None,
            "verified_by": rule.verified_by,
        }

    async def get_informatics_specifications(
        self, locale: str = "kk", year: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        query = (
            ...
            .options(
                selectinload(ExamSpecification.sections).selectinload(SpecificationSection.topics)
            )
            .join(Subject)
        ...
        for spec in specs:
            ...
            title = (
                spec.title_kk
                if locale == "kk"
                else (spec.title_ru if locale == "ru" else spec.title_en)
            )
            ...
            for sec in spec.sections:
                sec_title = (
                    sec.title_kk
                    if locale == "kk"
                    else (sec.title_ru if locale == "ru" else sec.title_en)
                )
                ...
                for top in sec.topics:
                    top_title = (
                        top.title_kk
                        if locale == "kk"
                        else (top.title_ru if locale == "ru" else top.title_en)
                    )
                    topics_data.append(
                        {
                            "id": top.id,
