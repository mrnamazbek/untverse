...
from sqlalchemy import String, Boolean, Integer, Text, ForeignKey, JSON, DateTime, Date
...
class ExamType(Base, TimestampMixin):
    ...
    specifications: Mapped[List["ExamSpecification"]] = relationship(
        "ExamSpecification", back_populates="exam_type"
    )
    current_rules: Mapped[List["CurrentUntRule"]] = relationship(
        "CurrentUntRule", back_populates="exam_type"
    )
...
class Subject(Base, TimestampMixin):
    ...
    specifications: Mapped[List["ExamSpecification"]] = relationship(
        "ExamSpecification", back_populates="subject"
    )
    bank_questions: Mapped[List["BankQuestion"]] = relationship(
        "BankQuestion", back_populates="subject"
    )
...
class ExamSpecification(Base, TimestampMixin):
    ...
    exam_type_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("exam_types.id", ondelete="CASCADE"), index=True, nullable=False
    )
    ...
    status: Mapped[str] = mapped_column(
        String(30), default=SpecificationStatus.ACTIVE, nullable=False, index=True
    )
    ...
    sections: Mapped[List["SpecificationSection"]] = relationship(
        "SpecificationSection",
        back_populates="specification",
        cascade="all, delete-orphan",
        order_by="SpecificationSection.order_index",
    )
...
class SpecificationSection(Base, TimestampMixin):
    ...
    specification_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("exam_specifications.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    ...
    title_en: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    weight_percentage: Mapped[int] = mapped_column(
        Integer, default=15, nullable=False
    )  # approx share of exam
    question_count_est: Mapped[int] = mapped_column(Integer, default=8, nullable=False)
    order_index: Mapped[int] = mapped_column(Integer, default=0, nullable=False)

    specification: Mapped["ExamSpecification"] = relationship(
        "ExamSpecification", back_populates="sections"
    )
    topics: Mapped[List["SpecificationTopic"]] = relationship(
        "SpecificationTopic",
        back_populates="section",
        cascade="all, delete-orphan",
        order_by="SpecificationTopic.order_index",
    )
...
class SpecificationTopic(Base, TimestampMixin):
    ...
    section_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("specification_sections.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    ...
    learning_objectives: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # list of NCT learning outcome codes (e.g. 10.1.2.1)
    order_index: Mapped[int] = mapped_column(Integer, default=0, nullable=False)

    section: Mapped["SpecificationSection"] = relationship(
        "SpecificationSection", back_populates="topics"
    )
    bank_questions: Mapped[List["BankQuestion"]] = relationship(
        "BankQuestion", back_populates="specification_topic"
    )
...
class CurrentUntRule(Base, TimestampMixin):
    ...
    verified_by: Mapped[str] = mapped_column(
        String(100), default="NTC_Official_2026", nullable=False
    )
