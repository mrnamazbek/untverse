...
class LocalizationGlossary(Base, TimestampMixin):
    ...
    context: Mapped[Optional[str]] = mapped_column(
        String(255), default="educational_unt", nullable=True
    )
    source: Mapped[str] = mapped_column(
        String(100), default="NTC_KZ", nullable=False
    )  # NTC_KZ, MES_KZ, ACADEMIC
    approved: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False, index=True)
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    __table_args__ = (Index("idx_glossary_search", "kk", "ru", "en"),)
