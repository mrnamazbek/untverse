...
class XpTransaction(Base, TimestampMixin):
    ...
    )  # quiz_passed, lesson_read, coding_task, streak_bonus, daily_quest, achievement
    reference_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)

    user: Mapped["User"] = relationship("User", back_populates="xp_transactions")

    __table_args__ = (Index("idx_xp_transactions_user_date", "user_id", "created_at"),)
...
class Achievement(Base, TimestampMixin):
    ...
    )  # streaks, quizzes, coding, mastery, speed
    xp_reward: Mapped[int] = mapped_column(Integer, default=100, nullable=False)
    condition_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # streak_days, total_quizzes, perfect_quizzes, python_tasks, level_reached
...
class DailyMission(Base, TimestampMixin):
    ...
    mission_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # answer_questions, complete_quiz, solve_coding, review_mistakes, read_lesson
...
class UserMission(Base, TimestampMixin):
    ...
    mission_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("daily_missions.id", ondelete="CASCADE"), index=True, nullable=False
    )
