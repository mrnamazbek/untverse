...
class NewsArticle(Base, TimestampMixin):
    ...
    category: Mapped[str] = mapped_column(
        String(50), default=NewsCategory.UNT, nullable=False, index=True
    )
    ...
    translations: Mapped[List["NewsTranslation"]] = relationship(
        "NewsTranslation", back_populates="article", cascade="all, delete-orphan"
    )
    versions: Mapped[List["NewsVersion"]] = relationship(
        "NewsVersion",
        back_populates="article",
        cascade="all, delete-orphan",
        order_by="NewsVersion.version_number.desc()",
    )
    sources_rel: Mapped[List["NewsSource"]] = relationship(
        "NewsSource", back_populates="article", cascade="all, delete-orphan"
    )
...
class NewsTranslation(Base, TimestampMixin):
    ...
    news_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("news_articles.id", ondelete="CASCADE"), index=True, nullable=False
    )
    ...
    __table_args__ = (Index("idx_news_translations_lookup", "news_id", "locale", unique=True),)
...
class NewsSource(Base, TimestampMixin):
    ...
    source_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("sources.id", ondelete="CASCADE"), index=True, nullable=False
    )
