...
class OfficialStatus(str):
    ...
    OFFICIAL_SPECIFICATION_BASED = (
        "official_spec_based"  # Modeled directly on NTC specification criteria
    )
...
class BankQuestion(Base, TimestampMixin):
    ...
    uuid_str: Mapped[str] = mapped_column(
        String(36), default=lambda: str(uuid.uuid4()), unique=True, index=True, nullable=False
    )

    subject_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("subjects.id", ondelete="RESTRICT"), index=True, nullable=False
    )
    specification_topic_id: Mapped[Optional[int]] = mapped_column(
        Integer,
        ForeignKey("specification_topics.id", ondelete="SET NULL"),
        index=True,
        nullable=True,
    )
    topic_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("topics.id", ondelete="SET NULL"), index=True, nullable=True
    )

    question_type: Mapped[str] = mapped_column(
        String(50), default="single_choice", nullable=False, index=True
    )
    difficulty: Mapped[str] = mapped_column(
        String(5), default=QuestionDifficulty.B, nullable=False, index=True
    )
    difficulty_score: Mapped[float] = mapped_column(
        Float, default=0.50, nullable=False
    )  # 0.00 to 1.00 empirical difficulty
    official_status: Mapped[str] = mapped_column(
        String(50), default=OfficialStatus.OFFICIAL_SPECIFICATION_BASED, nullable=False, index=True
    )
    original_language: Mapped[str] = mapped_column(
        String(10), default="kk", nullable=False
    )  # kk, ru, en
    ...
    )  # sha256 of canonical text representation

    subject: Mapped["Subject"] = relationship("Subject", back_populates="bank_questions")
    specification_topic: Mapped[Optional["SpecificationTopic"]] = relationship(
        "SpecificationTopic", back_populates="bank_questions"
    )
    ...
    provenance_records: Mapped[List["QuestionProvenance"]] = relationship(
        "QuestionProvenance", back_populates="question", cascade="all, delete-orphan"
    )
    solutions: Mapped[List["BankSolution"]] = relationship(
        "BankSolution", back_populates="question", cascade="all, delete-orphan"
    )
...
class QuestionVersion(Base, TimestampMixin):
    ...
    question_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("bank_questions.id", ondelete="CASCADE"), index=True, nullable=False
    )
...
class QuestionTranslation(Base, TimestampMixin):
    ...
    translation_source: Mapped[str] = mapped_column(
        String(50), default="official", nullable=False
    )  # official, human, ai, hybrid
    translation_status: Mapped[str] = mapped_column(
        String(30), default="published", nullable=False
    )  # published, validated, needs_review, draft
...
class QuestionBankOption(Base, TimestampMixin):
    ...
    translations: Mapped[List["QuestionBankOptionTranslation"]] = relationship(
        "QuestionBankOptionTranslation", back_populates="option", cascade="all, delete-orphan"
    )
...
class QuestionBankOptionTranslation(Base, TimestampMixin):
    ...
    option_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("question_bank_options.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    locale: Mapped[str] = mapped_column(String(10), nullable=False, index=True)  # kk, ru, en
    ...
    option: Mapped["QuestionBankOption"] = relationship(
        "QuestionBankOption", back_populates="translations"
    )

    __table_args__ = (Index("idx_option_translations_locale", "option_id", "locale", unique=True),)
...
class QuestionProvenance(Base, TimestampMixin):
    ...
    source_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("sources.id", ondelete="RESTRICT"), index=True, nullable=False
    )
    ...
    copyright_status: Mapped[str] = mapped_column(
        String(50), default="public_educational_use", nullable=False
    )
    license_type: Mapped[str] = mapped_column(
        String(50), default="NTC_Public_Sample", nullable=False
    )
    reuse_allowed: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    official_status: Mapped[str] = mapped_column(
        String(50), default=OfficialStatus.OFFICIAL_SAMPLE, nullable=False
    )
    content_hash: Mapped[str] = mapped_column(String(64), nullable=False)

    question: Mapped["BankQuestion"] = relationship(
        "BankQuestion", back_populates="provenance_records"
    )
...
class BankSolution(Base, TimestampMixin):
    ...
    approach_type: Mapped[str] = mapped_column(
        String(50), default="standard_analytical", nullable=False
    )  # standard_analytical, fast_exam_trick, python_script
    ...
    translations: Mapped[List["BankSolutionTranslation"]] = relationship(
        "BankSolutionTranslation", back_populates="solution", cascade="all, delete-orphan"
    )
...
class BankSolutionTranslation(Base, TimestampMixin):
    ...
    solution_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("solutions.id", ondelete="CASCADE"), index=True, nullable=False
    )
    locale: Mapped[str] = mapped_column(String(10), nullable=False, index=True)  # kk, ru, en
...
class Tag(Base, TimestampMixin):
    ...
    )  # concept, year, algorithm, exam_trap
...
class QuestionTag(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_question_tag_unique", "question_id", "tag_id", unique=True),)
