...
class User(Base, TimestampMixin):
    ...
    role: Mapped[str] = mapped_column(
        String(50), default=UserRoleEnum.STUDENT.value, nullable=False, index=True
    )
    ...
    spaced_cards: Mapped[List["SpacedRepetitionCard"]] = relationship(
        "SpacedRepetitionCard", back_populates="user", cascade="all, delete-orphan"
    )
...
class UserProfile(Base, TimestampMixin):
    ...
    rank_title: Mapped[str] = mapped_column(
        String(100), default="Новичок Информатики", nullable=False
    )
    streak_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)

    user: Mapped["User"] = relationship("User", back_populates="profile")

    __table_args__ = (Index("idx_user_profiles_xp_rank", "total_xp", "current_level"),)
