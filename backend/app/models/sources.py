...
class Source(Base, TimestampMixin):
    ...
    source_type: Mapped[str] = mapped_column(
        String(50), default="official_portal", nullable=False
    )  # official_portal, rss_feed, ministry, specification_doc
    authority_level: Mapped[str] = mapped_column(
        String(50), default=SourceAuthorityLevel.OFFICIAL_PRIMARY, nullable=False, index=True
    )
    ...
    question_provenance_records: Mapped[List["QuestionProvenance"]] = relationship(
        "QuestionProvenance", back_populates="source"
    )
...
class SourceDocument(Base, TimestampMixin):
    ...
    content_type: Mapped[str] = mapped_column(
        String(50), default="text/html", nullable=False
...
class IngestionRun(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(30), default=IngestionRunStatus.RUNNING, nullable=False, index=True
    )
    ...
    items: Mapped[List["IngestionItem"]] = relationship(
        "IngestionItem", back_populates="run", cascade="all, delete-orphan"
    )
...
class IngestionItem(Base, TimestampMixin):
    ...
    run_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("ingestion_runs.id", ondelete="CASCADE"), index=True, nullable=False
    )
    item_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # news, question, specification, rule
