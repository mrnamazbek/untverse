...
class CodingTask(Base, TimestampMixin):
    ...
    description: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # Markdown description with constraints & I/O format
    ...
    submissions: Mapped[List["CodingSubmission"]] = relationship(
        "CodingSubmission", back_populates="task", cascade="all, delete-orphan"
    )
...
class TestCase(Base, TimestampMixin):
    ...
    task_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("coding_tasks.id", ondelete="CASCADE"), index=True, nullable=False
    )
...
class CodingSubmission(Base, TimestampMixin):
    ...
    )  # accepted, wrong_answer, runtime_error, timeout, forbidden_syntax
    ...
    __table_args__ = (Index("idx_coding_submissions_user_task", "user_id", "task_id"),)
