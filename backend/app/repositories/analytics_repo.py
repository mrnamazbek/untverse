...
class AnalyticsRepository:
    ...
    async def record_mistake(self, user_id: int, question_id: int):
        res = await self.session.execute(
            select(MistakeLog).where(
                MistakeLog.user_id == user_id, MistakeLog.question_id == question_id
            )
        )
    ...
    async def get_due_srs_cards(self, user_id: int, limit: int = 20) -> List[dict]:
        ...
        res = await self.session.execute(
            ...
            .options(selectinload(SpacedRepetitionCard.question).selectinload(Question.options))
            .where(
                SpacedRepetitionCard.user_id == user_id, SpacedRepetitionCard.next_review_at <= now
            )
    ...
    async def record_study_session(self, user_id: int, duration_seconds: int, activity_type: str):
        session = StudySession(
            user_id=user_id, duration_seconds=duration_seconds, activity_type=activity_type
        )
    ...
    async def get_student_dashboard_metrics(self, user_id: int) -> dict:
        ...
        quizzes_res = await self.session.execute(
            select(
                func.count(QuizAttempt.id), func.coalesce(func.avg(QuizAttempt.percentage), 0.0)
        ...
        coding_res = await self.session.execute(
            select(func.count(func.distinct(CodingSubmission.task_id))).where(
        ...
        mistakes_res = await self.session.execute(
            select(func.count(MistakeLog.id)).where(
                MistakeLog.user_id == user_id, MistakeLog.is_resolved == False
            )
        )
        ...
        sorted_by_mastery = sorted(
            all_masteries, key=lambda x: x["mastery_percentage"], reverse=True
        )
        strongest = sorted_by_mastery[:3] if sorted_by_mastery else []
        weakest = sorted(
            [m for m in all_masteries if m["total_answered"] > 0],
        ...
        avg_mastery = sum(m["mastery_percentage"] for m in all_masteries) / max(
            1, len(all_masteries)
        )
